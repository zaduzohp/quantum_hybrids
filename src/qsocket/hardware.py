"""Readout from a measurement histogram, and the IQM hardware estimator.

expectations_from_counts turns one histogram into <Z_i> for every qubit;
expectations_on_backend transpiles, executes in batches and returns those values in input
...
def zz_expectation_from_counts(counts: dict[str, int], i: int, j: int, n_qubits: int) -> float:
...
def connected_correlation(counts: dict[str, int], n_qubits: int) -> float:
    ...
    The connected part, not the raw correlator: <Z_i Z_j> alone is nonzero for a product
    state whenever the single-qubit values are, so it would not distinguish an entangling
    circuit from the product control.
    """
